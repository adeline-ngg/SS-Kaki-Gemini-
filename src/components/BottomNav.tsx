...
export const BottomNav: React.FC<BottomNavProps> = ({
  ...
  // Map internal screens to the active tab
  ...
  return (
    <nav
      ...
      className="flex-shrink-0 z-20 bg-[#F9F6F2] border-t border-[#EDE0D4] px-4 py-2 shadow-xs"
    >
      <div className="flex items-center justify-around gap-2 max-w-md mx-auto">
        ...
        <button
          ...
          className={`flex-1 flex flex-col items-center justify-center py-1.5 px-2 rounded-2xl transition-all touch-manipulation min-h-[50px] active:scale-95 ${
            activeTab === 'today'
              ? 'text-[#CB8570] font-bold bg-[#EDE0D4]/40'
              : 'text-[#2D2C2A]/60 hover:text-[#2D2C2A]'
          }`}
        >
          <Sparkles className={`w-5 h-5 mb-0.5 ${activeTab === 'today' ? 'text-[#CB8570] stroke-[2.4]' : 'stroke-[1.8]'}`} />
        ...
        >
          <div className="relative">
            <Mic className={`w-5 h-5 mb-0.5 ${activeTab === 'talk' ? 'text-[#CB8570] stroke-[2.4]' : 'stroke-[1.8]'}`} />
            {activeTab === 'talk' && (
        ...
        >
          <Compass className={`w-5 h-5 mb-0.5 ${activeTab === 'my-world' ? 'text-[#CB8570] stroke-[2.4]' : 'stroke-[1.8]'}`} />
          <span className={`leading-none ${isLargeText ? 'text-xs font-bold' : 'text-[11px] font-semibold'}`}>
            World · 我的
          </span>
        </button>
